...
do.many.sims <- function(pars,num.sims,scenario.name){
  ...
  file.name <- paste(scenario.name,"/pars_scenario",scenario.name,
                     ".RData",sep="")
  save(pars,file=file.name)
  for(ii in 1:num.sims){
    tmp.data <- do.single.sim(pars)
    file.name <- paste(scenario.name,"/simdata_scenario",scenario.name,
                       "_num",ii,".csv",sep="")
...
pars <- list(
  ...
  num.items=31, #number of items
  items.study0=c(1:16,20,18), #items in study0 #add 18
  items.study1=c(16:31,12,14), #items in study1 #add 14
  item.difficulties= seq(-2.1,2.1,length.out=31), #item difficulties
  item.discriminations=rep(1,31) #item discriminations
)
...
do.many.sims(pars,scenario.name="TEST",num.sims=10)
